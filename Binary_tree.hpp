...
class node
{
	...
	node(void) :  left(NULL), right(NULL), parrent(NULL), pair(NULL)
	{
	}

	node(pair_type const &p) :  left(NULL), right(NULL), parrent(NULL), pair(p)
	{
	}
	~node(void) {}
